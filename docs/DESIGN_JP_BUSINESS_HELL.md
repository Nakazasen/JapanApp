# ﾄ雪ｻ蟻 ng盻･c ti蘯ｿng Nh蘯ｭt (譌･譛ｬ隱槫慍迯・ - Technical Design
...
2. **Service Layer (`frontend/services/jp_business_hell_ai.py`)**:
   ...
   - The requests are sent to the `AIRouter`, which decides the optimal AI provider based on the `task_routing_policy.yaml`.
   - The service also integrates with `JapaneseWorkLearningMemory` to persist feedback and generate Spaced Repetition (SRS) items.
...
## Boss Unlock Logic
- Computed dynamically in the UI Dashboard.
- Iterates over `scores_by_gate`.
- Unlocks the "Final Boss" gate only if `overall_avg >= 70` and `total_drills >= 5`.

## Safe Evaluation
- Harness script `harness/jp_hell_product_eval.py` simulates UI interactions without requiring API keys.
- Falls back to `offline_provider` for safe, deterministic CI/CD testing.
