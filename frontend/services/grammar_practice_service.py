...
class GrammarPracticeService(BaseService):
    def list_items(self) -> List[Dict[str, Any]]:
        try:
            with get_session() as session:
                items = session.exec(select(GrammarPracticeItem)).all()
                return [{"id": i.id, "title": i.title, "source": i.source, "question_count": 0} for i in items]
        except Exception as e:
            print(f"[ERROR GrammarPracticeService] list_items: {e}")
            return []

    def get_item_detail(self, item_id: int) -> Optional[Dict[str, Any]]:
        try:
            with get_session() as session:
                item = session.get(GrammarPracticeItem, item_id)
                if not item: return None
                return {
                    "id": item.id, "title": item.title, "source": item.source, "transcript": item.transcript,
            ...
            print(f"[ERROR GrammarPracticeService] get_item_detail: {e}")
            return None
...
_service = None
def get_grammar_practice_service():
    global _service
    if _service is None: _service = GrammarPracticeService()
    return _service
