...
class KanjiPracticeService(BaseService):
    def list_items(self) -> List[Dict[str, Any]]:
        try:
            with get_session() as session:
                items = session.exec(select(KanjiPracticeItem)).all()
                return [{"id": i.id, "title": i.title, "source": i.source, "question_count": 0} for i in items]
        except Exception as e:
            print(f"[ERROR KanjiPracticeService] list_items: {e}")
            return []

    def get_item_detail(self, item_id: int) -> Optional[Dict[str, Any]]:
        try:
            with get_session() as session:
                item = session.get(KanjiPracticeItem, item_id)
            ...
            print(f"[ERROR KanjiPracticeService] get_item_detail: {e}")
            return None
...
_service = None
def get_kanji_practice_service():
    global _service
    if _service is None: _service = KanjiPracticeService()
    return _service
