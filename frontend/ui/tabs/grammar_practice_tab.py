from frontend.services.grammar_practice_service import get_grammar_practice_service
...
class GrammarPracticeTab(QWidget):
    def __init__(self):
        super().__init__()
        self.service = get_grammar_practice_service()
    ...
    def _init_ui(self):
        ...
        sidebar_lay.addWidget(QLabel("🔖 LUYỆN NGỮ PHÁP (JT4Y)"))
        ...
        self.grammar_btn = QPushButton("📖 Ngữ pháp")
        self.grammar_btn.clicked.connect(self._show_vocab_dialog)
        self.grammar_btn.setStyleSheet(f"QPushButton {{ background: {ThemeColors.BG_TERTIARY}; border: 1px solid {ThemeColors.BORDER}; border-radius: 15px; padding: 5px 15px; color: {ThemeColors.TEXT_PRIMARY}; }}")
        nav_bar.addWidget(self.grammar_btn)
    ...
    def _show_vocab_dialog(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("📖 Ngữ pháp & Từ vựng")
