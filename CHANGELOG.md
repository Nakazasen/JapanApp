...
- **Japanese Menu:** Added missing "Từ vựng" and "Ngữ pháp" tabs to Japanese context.
...
- **📄 PDF Vocabulary Import (Gemini Vision):**
  ...
  - **Data Ingestion**: Successfully imported 50 words from "DANH SÁCH TỪ.pdf" and 120 idioms from "120 THÀNH NGỮ + CỤM TỪ.pdf" into the `EnVocabItem` database.

### Changed

- **🎴 Enhanced Flashcards:**
  - **Rich Content**: Now displays examples (`GrammarExample`), usage notes, and common mistakes directly on the back of the card.
  - **Game-like Unlock**: Added a prominent **"✅ Đã hiểu!"** button that marks a unit as Mastered and immediately unlocks subsequent nodes on the map.
...
- **🗺️ Learning Map (MVP Complete):**
  - **Gamified Grammar Learning**: Visual map of 189 grammar units across 5 Fantasy-themed regions (A1-C1).
  - **4 Node States**: Locked 🔒, Available 📘, Learning 📖, Mastered ⭐ with distinct visual styling.
  - **Interactive Features**: Zoom/pan navigation, bezier path lines, progress rings, glow effects.
  - **GrammarTab Integration**: Toggle button "🗺️ Bản đồ" to switch between List View and Map View.
  - **Node Click → Flashcard**: Clicking a node opens the corresponding grammar flashcard.
  - **New Files**:
...
- **✨ Vocabulary Tinder Mode (Complete):**
...
- **🎮 Gamification & UI:**
  - **Kanji Dashboard Wall**: Fixed "Bản đồ công trình" not updating. Now counts 'Reviewing' items (0.5 points) towards progress.
  - **Auto-Refresh**: Dashboard now automatically updates after study sessions.
  - **Vietnamese TTS**: Added `strip_emojis` to prevent reading icon names (e.g., "bông hoa", "bóng đèn") during text-to-speech.
- **🚀 Stability:**
